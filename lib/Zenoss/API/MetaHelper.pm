...
Zenoss::API::MetaHelper - Internal package that helps create Meta Methods

=head1 DESCRIPTION

B<This is not for public consumption.>

This is a helper to install Meta Methods.  For example, Zenoss::API::Router::Tree has methods
in it that are installed to various routers.  IE Zenoss::API::Router::Network implements methods
from Zenoss::API::Router::Tree, but the methods are renamed to be in line with the calling router.

Breaking that down:

Zenoss::API::Router::Tree has a method called addNode, but it will appear as network_addNode in
Zenoss::API::Router::Network.
