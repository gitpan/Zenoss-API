...
sub BUILD {
    ...
    my @router_install;
    foreach (keys %router) {
        push(@router_install, "Zenoss::API::Router::$_");
    }
...
sub _router_request {
    ...
    # Return Zenoss::API::Handler object
    return Zenoss::API::Response->new(
        {
...
Zenoss::API::Router - Internal package that does the processing and sending and receiving Zenoss API calls

=head1 DESCRIPTION

B<This is not for public consumption.>

In brief, this package processes requests to the Zensos API and returns a Zenoss::API:Response object.
However, this is considered internal and would be called by one of the high level packages - such as
Zenoss::API.
